...
## R1-R6 根因框架（防止 rules 膨胀）

> 每次任务偏差必须归因后再决定是否改规范。不是因为"看起来不够完整"就加规则。
...
- **R5 / R6 不触发规范进化**：代码 bug 和外部因素不是加规则的理由
- **R1-R4 才是补规则的合法依据**：只有规范本身的问题才需要改规范

### 使用方法

1. 任务出现偏差时，先归因到 R1-R6
2. 根据根因决定动作
3. 记录归因结果（防止重复犯错）
...
```
偏差：agent 跳过了验证步骤
归因：R4（Review 漏洞）— 验证步骤没有强制检查
动作：在 verification-before-completion 中增加强制检查点
```

```
偏差：构建失败
归因：R5（代码 bug）— 逻辑错误，非规范问题
动作：修代码，不动规范
```
