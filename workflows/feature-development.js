 ...
 *
 * 流程：
 * 1. 并行生成 4 份文档（需求、分析、设计、任务）
 * 2. 架构审查
 * 3. 代码开发（用户手动）
 * 4. 代码审查
 * 5. 完成验证
 */
async function featureDevelopment(args) {
  const { featureName, featureDescription, budget } = args

  // Budget 控制：默认 200k
  const tokenBudget = budget?.total || 200000

  if (!featureName) {
    throw new Error('缺少功能名称，请提供 featureName 参数')
  }

  log(`开始功能开发工作流：${featureName}`)

  // ========== 阶段一：生成文档（prompt-chaining，按依赖顺序） ==========
  // 官方依据：Building effective agents · Parallelization 仅适用「独立子任务」；
...
包含：功能边界、异常场景、数据流、API 设计、权限设计。`,
  ...
  // fail-loud：上游缺失直接中断，绝不用占位符继续（官方：无法验证就不发布）
...
输出审查报告，包含：状态（PASS/FAIL/CONDITIONAL）、发现列表、总结。`,
  ...
  // ========== 阶段三：代码审查 ==========
  // 官方依据：审查者应在「fresh context 只看 diff + 标准」，与产出代码的推理隔离；
  // 且「只报影响正确性/需求的问题，避免过度工程」。
  phase('代码审查')
...
输出审查报告，包含：状态（PASS/FAIL/CONDITIONAL）、发现列表、总结。`,
  ...
  // ========== 阶段四：完成验证（确定性 gate） ==========
  // 官方依据：「Give Claude a way to verify its work」— 验证必须基于可返回
  // pass/fail 的确定性信号（build/test 退出码），而非让 agent 自述「应该过了」。
  phase('完成验证')
